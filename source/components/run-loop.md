A maior parte do codigo do Ember que você escrever em sua aplicação irá 
fazer parte de um run loop. O run loop é usado para carregar e ordernar (ou reordenar)
da maneira mais eficiente e efetiva.

Ele faz isso programando diversas ações em uma fila. Essas filas tem uma prioridade,
e então sendo processadas pela conclusão das tarefas anteriores

Para apps mais basicos, você não vai precisar entender o run loop ou mesmo 
usar ele diretamente. Todo o cenario comum esta bem abstraido e funcionando,
então você não precisa trabalhar no loop diretamente.

O caso mais comum de uso de run loope é na integração com alguma API non-Ember,
isso inclue alguns tipos de callback asymchrono. Por exemplo:
